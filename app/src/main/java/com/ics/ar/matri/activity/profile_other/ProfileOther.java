package com.ics.ar.matri.activity.profile_other;
...
import com.ics.ar.matri.view.ProfilePageProfilePhotoImageView;
...
public class ProfileOther extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // parmsVisitor.put(Consts.USER_ID, loginDTO.getData().getId());
        parmsVisitor.put(Consts.USER_ID, prefrence.getKEY_UID());

        if (getIntent().hasExtra(Consts.USER_DTO)) {
            userDTO = (UserDTO) getIntent().getSerializableExtra(Consts.USER_DTO);
            parmsVisitor.put(Consts.VISITOR_ID, userDTO.getId());
            parms.put(Consts.USER_ID, userDTO.getId());
            ...
            public void backResponse(boolean flag, String msg, JSONObject response) {
                ...
                if (flag) {
                    try {
                        ...
                        if (imageDatalist.size() > 0) {
                            rlGallaryClick.setVisibility(View.VISIBLE);
                            tvImageCount.setText(imageDatalist.size() + "");
                        } else {
                            rlGallaryClick.setVisibility(View.GONE);
